...
class Admin {
    ...
    fun adminChat(css: CommandSourceStack, @Argument("text") text: Array<String>) {
        if(css.sender is Player) {
            ...
            ChatUtility.broadcastAdmin("<head:${player.name}> <dark_red>${player.name}<white>: ${text.joinToString(" ")}", false)
        }
    }
    ...
    fun devChat(css: CommandSourceStack, @Argument("text") text: Array<String>) {
        if(css.sender is Player) {
            val player = css.sender as Player
            ChatUtility.broadcastDev("<head:${player.name}> <gold>${player.name}<white>: ${text.joinToString(" ")}", false)
        }
    }
}