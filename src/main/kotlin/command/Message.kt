...
private data class LastMessanger(val sender: UUID, val timeStamp: Long)

@CommandContainer
@Suppress("unused", "unstableApiUsage")
class Message {
    private val lastConversationPartner = mutableMapOf<UUID, LastMessanger>()
    /**
    ...
    fun reply(css: CommandSourceStack, @Argument("text") text: Array<String>) {
        if(css.sender is Player) {
            ...
            if (lastSender != null && System.currentTimeMillis() - lastSender.timeStamp <= REPLY_TIMEOUT_SECONDS * 1000) {
                ...
                sender.sendMessage(
                    Formatting.restrictedTags.deserialize("<i><gray>Nobody has messaged you in a while, but don't worry we still love you <3 - Cloudie Admins</gray></i>")
                )
            }
        }
    }
    ...
    private fun sendMessage(sender: Player, recipient: Player, text: Array<String>) {
        ...
        sender.sendMessage(Formatting.restrictedTags.deserialize("<i><cloudiecolor>You</cloudiecolor> -> <yellow>${recipient.name}</yellow>: $message</i>"))
        recipient.sendMessage(Formatting.restrictedTags.deserialize("<i><yellow>${sender.name}</yellow> -> <cloudiecolor>You</cloudiecolor>: $message</i>"))

        lastConversationPartner[recipient.uniqueId] = LastMessanger(sender.uniqueId, System.currentTimeMillis())
        lastConversationPartner[sender.uniqueId] = LastMessanger(recipient.uniqueId, System.currentTimeMillis())
    }
