...
class RenameItem {
    ...
    fun renameItem(css: CommandSourceStack, name: Array<String>) {
        ...
        if (!player.hasLapisInInventory() && player.gameMode !== org.bukkit.GameMode.CREATIVE) {
        ...
        player.sendMessage(Formatting.allTags.deserialize("<cloudiecolor>Renamed item!"))
    ...
    fun resetItemName(css: CommandSourceStack) {
        ...
        player.inventory.itemInMainHand.setItemMeta(itemMeta)
        player.sendMessage(Formatting.allTags.deserialize("<cloudiecolor>Reset item name!"))
        player.playSound(RENAME_ITEM)
    }
}